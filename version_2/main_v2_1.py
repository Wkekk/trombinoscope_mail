...
def recherche():
    ...
    mail_photo = [] # Liste avec les mails des photos
    ...
    for i in range(len(resultat)):
        width_image = 140 # Largeur des images
        height_image = 100 # Hauteur des images
        # Ouverture de la photo
        # Vérification si image associée à une personne
        if os.path.exists(resultat[i].photo):
            img = Image.open(resultat[i].photo) # Ouverture de l'image
        else:
            img = Image.open(photo_path + "no_photo_available.jpg") # Ouverture "image no available" 

        # Vérifier si image au format portrait ou au format paysage
        w, h = img.size
        if h > w:
            width_image, height_image = height_image, width_image
        ...
        # Texte photo
        legende_photo = f"{resultat[i].genre} {resultat[i].nom} {resultat[i].prenom} \n({resultat[i].statut})"
        label_photo.append(tk.Label(frame_affichage, text=legende_photo))
        label_photo[i].grid(row=ligne_image+1, column=colonne_image)

        # Text mail
        mail_photo.append(tk.Button(frame_affichage, text=f"{resultat[i].mail}", fg="blue", relief="flat"))
        mail_photo[i].configure(command=lambda i=i:mailing(resultat[i].mail))
        mail_photo[i].grid(row=ligne_image+2, column=colonne_image)

        cnt_image += 1
        colonne_image += 1

        if cnt_image == 5: # 8 photos par lignes initialement
            cnt_image = 0
            ligne_image += 3 # initialement 2
            colonne_image = 0
